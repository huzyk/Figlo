...
const today=currentDateKey(),dailySeed=dailyPuzzleSeed('korony',today),dailyDifficulty=difficultyForGameDate('korony',today);
const isLocalTest=location.hostname==='localhost'||location.hostname==='127.0.0.1';
let currentPuzzle=null,productState=loadFigloState(today),state=createGameState(),history=[],started=false,tick=null,finished=false,gameMode='daily',practiceNumber=0,autoXEnabled=Boolean(productState.settings.autoX),elapsedMs=0,runningSince=null;
...
function render(){
  ...
  updateControls();updateFeedback(bad);
...
function finishGame(ms){finished=true;pauseTimer();elapsedMs=ms;runningSince=null;timer.textContent=fmt(ms);finalTime.textContent=`Czas ${timer.textContent}`;if(gameMode==='daily'||gameMode==='replay'){const previousBest=productState.games.korony.bestTimeMs,result=completeDailyGame('korony',{timeMs:ms,today});productState=result.state;const currentBest=productState.games.korony.bestTimeMs,isNewRecord=Number.isFinite(currentBest)&&currentBest===ms&&(!previousBest||ms<previousBest);$('#recordBadge').hidden=!isNewRecord;}else $('#recordBadge').hidden=true;if(gameMode==='daily')persistSession();refreshStats();}
...
function exposeTestHook(){if(!isLocalTest||!currentPuzzle)return;globalThis.__FIGLO_TEST__={getSolution:()=>[...currentPuzzle.solution],getDifficulty:()=>({level:dailyDifficulty,score:currentPuzzle.difficultyScore})};}

async function loadDailyPuzzle(){
  board.setAttribute('aria-busy','true');
  try{
    currentPuzzle=await generateCrownDailyAsync(dailySeed,dailyDifficulty);
    setRegions(currentPuzzle.regions);buildBoard();exposeTestHook();
    const alreadyCompleted=productState.daily.completedGames.includes('korony'),session=getCrownSession({today,seed:dailySeed});
    if(!alreadyCompleted&&session&&!session.finished){gameMode='daily';roundLabel.textContent='Dzisiaj';restoreSession(session);render();return;}
    if(alreadyCompleted){gameMode='replay';roundLabel.textContent='Replay';clearCrownSession(today);resetRound({persist:false});return;}
    gameMode='daily';roundLabel.textContent='Dzisiaj';resetRound({persist:true});
  }catch(error){console.error('korony daily generation failure',error);feedback.textContent='Nie udało się przygotować dzisiejszych Koron. Odśwież stronę.';feedback.className='feedback error';}
...
async function loadPracticePuzzle(){
  ...
  board.setAttribute('aria-busy','true');
  try{
    currentPuzzle=await generateCrownDailyAsync(practiceSeed());
    setRegions(currentPuzzle.regions);buildBoard();exposeTestHook();
    gameMode='practice';practiceNumber+=1;roundLabel.textContent=`Trening #${practiceNumber}`;
...
nextPuzzleBtn.addEventListener('click',async()=>{nextPuzzleBtn.disabled=true;nextPuzzleBtn.textContent='Generuję…';await new Promise(resolve=>requestAnimationFrame(resolve));try{await loadPracticePuzzle();nextPuzzleBtn.textContent='Nowa plansza treningowa';}catch(error){console.error(error);nextPuzzleBtn.textContent='Spróbuj ponownie';}nextPuzzleBtn.disabled=false;});
