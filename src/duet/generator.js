...
export const DUET_GENERATOR_VERSION = 1;
...
function validRowPatterns() {
  const rows = [];
  for (let mask = 0; mask < 1 << SIZE; mask++) {
    const row = Array.from({ length: SIZE }, (_, i) => ((mask >> i) & 1) ? A : B);
    if (row.filter(v => v === A).length !== SIZE / 2) continue;
    let triple = false;
    for (let i = 0; i <= SIZE - 3; i++) if (row[i] === row[i + 1] && row[i + 1] === row[i + 2]) triple = true;
...
function generateSolution(random) {
  ...
  const rowOrder = shuffle(ROW_PATTERNS, random);

  function search(row) {
    if (row === SIZE) return isSolved(board, { givens: [], relations: [] });
    for (const pattern of shuffle(rowOrder, random)) {
      for (let col = 0; col < SIZE; col++) board[indexOf(row, col)] = pattern[col];
      if (isPartialBoardValid(board, { relations: [] }) && search(row + 1)) return true;
      for (let col = 0; col < SIZE; col++) board[indexOf(row, col)] = EMPTY;
    }
    return false;
  }

  if (!search(0)) throw new Error('Nie udało się zbudować rozwiązania Duetu.');
  return [...board];
}
...
function allAdjacentPairs() {
  ...
  for (let row = 0; row < SIZE; row++) {
    for (let col = 0; col < SIZE; col++) {
      ...
      if (col + 1 < SIZE) pairs.push([a, indexOf(row, col + 1)]);
      if (row + 1 < SIZE) pairs.push([a, indexOf(row + 1, col)]);
    }
  }
  return pairs;
}

function createRelations(solution, random, count = 8) {
  return shuffle(allAdjacentPairs(), random).slice(0, count).map(([a, b]) => ({
    a,
    b,
    type: solution[a] === solution[b] ? REL_SAME : REL_DIFFERENT
  }));
}
...
function acceptable(puzzle) {
  ...
  const human = solveLikeHuman(puzzle);
  if (!human.solved) return null;
  return human;
}
...
export function generateDuetPuzzle(seed, { maxAttempts = 40 } = {}) {
  ...
  for (let attempt = 0; attempt < maxAttempts; attempt++) {
    ...
    const initial = createInitialPuzzle(solution, random);
    const puzzle = minimizePuzzle(initial, random);
