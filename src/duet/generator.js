...
const DIFFICULTY_PROFILES = {
  easy: { minScore:DAILY_MIN_SCORE, targetScore:25, maxScore:DAILY_MAX_SCORE },
  medium: { minScore:DAILY_MIN_SCORE, targetScore:30, maxScore:DAILY_MAX_SCORE },
  hard: { minScore:DAILY_MIN_SCORE, targetScore:36, maxScore:DAILY_MAX_SCORE }
};
...
export function generateDuetPuzzle(seed, { maxAttempts = 160, difficulty = 'medium' } = {}) {
  const normalizedSeed = String(seed); const profile = profileFor(difficulty); let best = null;
  for (let attempt = 0; attempt < maxAttempts; attempt++) { const attemptSeed = `${normalizedSeed}:${attempt}`; const random = seededRandom(hashSeed(`figlo:duet:${DUET_GENERATOR_VERSION}:${attemptSeed}`)); const solution = generateSolution(random); const puzzle = minimizePuzzle(createInitialPuzzle(solution, random), random); const human = acceptable(puzzle); if (!human) continue; const graded = gradeDifficulty(human); const candidate = { seed:normalizedSeed, resolvedSeed:attemptSeed, generatorVersion:DUET_GENERATOR_VERSION, requestedDifficulty:difficulty, puzzle, solution, difficulty:graded }; if (!isDailyCandidate(puzzle, graded, profile)) continue; if (!best || distanceFromTarget(graded.score, profile.targetScore) < distanceFromTarget(best.difficulty.score, profile.targetScore)) best = candidate; if (graded.score === profile.targetScore) return candidate; if (best && attempt >= BEST_CANDIDATE_EARLY_RETURN_ATTEMPT) return best; }
  if (best) return best;
