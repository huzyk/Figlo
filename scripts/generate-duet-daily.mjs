...
import { puzzleIdFor } from '../src/domain/completion.js';
...
function parseArgs() {
  ...
  if (!/^\d{4}-\d{2}-\d{2}$/.test(result.start || '')) throw new Error('Użycie: node scripts/generate-duet-daily.mjs --start YYYY-MM-DD [--days N]');
  if (!Number.isInteger(result.days) || result.days < 1) throw new Error('--days musi być dodatnią liczbą całkowitą.');
  return result;
}
function addDays(dateKey, offset) { const [y,m,d]=dateKey.split('-').map(Number); return new Date(Date.UTC(y,m-1,d+offset)).toISOString().slice(0,10); }

const { start, days } = parseArgs();
await fs.mkdir(outputDir, { recursive: true });
let totalMs=0,maxMs=0,slowestDate=null;
...
for (let offset=0; offset<days; offset++) {
  ...
  if(!human.solved||human.steps.some(step=>step.rule==='guess'||step.rule==='lookahead')) throw new Error(`Duet ${date}: human solver nie rozwiązuje planszy bez zgadywania.`);

  const version=generated.generatorVersion||DUET_GENERATOR_VERSION;
  const record={
    date,
    game:'duet',
    version,
  ...
  await fs.writeFile(path.join(outputDir,`${date}.json`),`${JSON.stringify(record,null,2)}\n`,'utf8');
