...
function renderDate() {
  const formatted = new Intl.DateTimeFormat('pl-PL',{weekday:'long',day:'numeric',month:'long'}).format(todayDate);
  $('#todayDate').textContent = formatted.charAt(0).toUpperCase()+formatted.slice(1);
}
function markGame(id, cardId, statusId) {
  const done=state.daily.completedGames.includes(id); const card=$(cardId); const status=$(statusId);
  if(done){status.textContent='Zagraj ponownie';card.classList.add('completed');card.setAttribute('aria-label',`${id==='korony'?'Korony':'Duet'} ukończone dzisiaj. Zagraj ponownie`);}
}
function renderDailyProgress() {
  $('#progressRing').textContent=`${completedCount}/${requiredCount}`;
  const percent=requiredCount?completedCount/requiredCount:0;
  ...
  markGame('korony','#crownsCard','#crownsStatus');
  if($('#duetCard')) markGame('duet','#duetCard','#duetStatus');
}
...
renderDate();renderDailyProgress();renderStats();renderWeek();renderGamesSummary();wireMobileMenu();
