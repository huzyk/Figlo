...
test('new day clears Duet session but preserves game state',()=>{localStorage.clear();saveDuetSession({date:'2026-08-24',seed:'a',board:Array(36).fill(1)},'2026-08-24');const next=loadFigloState('2026-08-25');assert.equal(next.sessions.duet.date,null);assert.deepEqual(next.sessions.duet.board,[]);assert.deepEqual(next.daily.requiredGames,['korony','duet','bloki','latarnie']);});
