...
async function solveDuet(page) {
  const record = await page.evaluate(
    async date => await (await fetch(`/data/duet/${date}.json`)).json(),
    DATE
  );
...
test('Duet move and undo survive reload', async ({ page }, info) => {
  ...
  const undo = info.project.name === 'mobile-chromium'
    ? page.locator('#mobileUndo')
    : page.locator('#undo');
...
test('Duet cells keep equal size after symbols render', async ({ page }) => {
  ...
  const sizes = await page.locator('.duet-cell').evaluateAll(cells =>
    cells.map(cell => {
