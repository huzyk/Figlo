...
test('Bloki clues can omit size or shape while keeping uniqueness',()=>{const puzzles=Array.from({length:20},(_,i)=>generateBloki({seed:`clue-mix-${i}`,difficulty:'medium'}));assert.ok(puzzles.some(p=>p.clues.some(c=>c.area==null||c.shape==null||c.shape==='any')));for(const puzzle of puzzles)assert.equal(countSolutions(puzzle,2),1);});
